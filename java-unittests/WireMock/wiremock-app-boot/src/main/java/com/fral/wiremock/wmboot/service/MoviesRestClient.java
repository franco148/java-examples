...
import com.fral.wiremock.wmboot.constants.MoviesAppConstants;
import com.fral.wiremock.wmboot.dto.Movie;
import com.fral.wiremock.wmboot.exception.MovieErrorResponse;
...
public class MoviesRestClient {
    ...
    public Movie addNewMovie(Movie newMovie) {
        ...
        try {
            movie = webClient.post().uri(MoviesAppConstants.ADD_MOVIE_V1)
                    .syncBody(newMovie)
                    .retrieve()
    ...
    public Movie updateMovie(Integer movieId, Movie movie) {
        ...
        try {
            updatedMovie = webClient.put().uri(MoviesAppConstants.MOVIE_BY_ID_PATH_PARAM_V1, movieId)
                    .syncBody(movie)
                    .retrieve()
                    .bodyToMono(Movie.class)
                    .block();
    ...
    public String deleteMovieById(Long movieId) {
       ...
       String response;
        try {
            response = webClient.delete().uri(MoviesAppConstants.MOVIE_BY_ID_PATH_PARAM_V1, movieId)
                    .retrieve()
